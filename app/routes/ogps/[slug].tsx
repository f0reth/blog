  ...
  async (c) => {
    ...
    const svg = await satori(
      <div tw={"w-full h-full bg-[#E0EFFF] flex p-9"}>
        <div tw={"w-full flex flex-col justify-end bg-white/85 rounded-3xl border-solid"}>
          <div tw={"w-full flex flex-1 items-center mt-10 px-34"}>
            <div tw={"flex justify-center flex-wrap text-[4.4rem]"}>{parsedTitle}</div>
          </div>
          <div tw={"w-full flex items-center justify-between px-18 mb-10 text-gray-900"}>
...
function buildGoogleFontUrl({ family, weight, text, display }: GoogleFontOptions) {
