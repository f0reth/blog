...
以下の記事でUnoCSSが紹介されていたので、気になって使ってみました。
...
この記事ではUnoCSSをメインで記述しますので、セットアップは以下の公式サイトを参考にしてください。
...
UnoCSSの設定ファイル`uno.config.ts`を作成します。

TailwindCSSを使う場合は、プリセットが先ほどインストールしたモジュールに入っています。
...
最後に`main.tsx`にUnoCSSをimportします。

TailwindCSS用のリセットCSSもimportします。
...
Attributify機能は、TailwindCSSを使っていてclassが長くなり、読みにくいときに便利です。
...
VS Codeを使っていたらUnoCSSの拡張機能が提供されていますので、インストールすると開発体験が良くなります。
...
今回設定した以外にも、Pure CSSによるアイコンやウェブフォントの設定など機能がたくさんあります。

<LinkCard url="https://unocss.dev/presets/icons" />

紹介した機能のコードは以下のGitHubに置いてます。
